...
const chartData = esgData.map(item => ({
  x: item.log_emissions,
  y: item.environment_score,
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white p-3 border border-gray-300 rounded shadow-lg">
        ...
        <p className="text-sm text-gray-600">Log Emissions: {data.x.toFixed(2)}</p>
        <p className="text-sm text-gray-600">Environmental Score: {data.y}</p>
...
// Function to determine dot color based on environment level
    ...
    <div className="w-full bg-white rounded-lg p-6 shadow-lg">
      <div className="max-w-4xl mx-auto mb-8">
        <h3 className="text-2xl font-semibold text-gray-800 mb-4 text-center">
          Environmental Score vs Emissions
        </h3>
      ...
      <div className="h-[600px]">
        ...
        >
          ...
          <XAxis 
            ...
            dataKey="x" 
            name="Log Emissions"
            label={{ value: 'Log Emissions (log₁₀ mtCO2e)', position: 'insideBottom', offset: -10 }}
            domain={[4, 9]}
          />
          <YAxis 
            type="number" 
            ...
            label={{ value: 'Environmental Score', angle: -90, position: 'insideLeft' }}
            domain={[0, 750]}
          />
          ...
          >
            {chartData.map((entry, index) => (
              <Cell key={`cell-${index}`} fill={getDotColor(entry.level)} />
            ))}
          </Scatter>
        </ScatterChart>
      </ResponsiveContainer>
